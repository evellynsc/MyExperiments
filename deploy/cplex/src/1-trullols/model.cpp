...
{
    // convert y and z into continuous vars in [0,1] and [0,tau], respectively
    IloConversion conv_y(env, y, ILOFLOAT);
    model.add(conv_y);
    //IloConversion conv_z(env, z, ILOFLOAT);  // NOTE: if z is float, then we don't need this
    //model.add(conv_z);
...
void Model::create_variables()
{
    ...
    // aux vars indicating the contribution of an access point to the objective function
    z = IloNumVarArray(env, instance->m, 0.0, instance->tau, ILOFLOAT);
    
    for (int j=0; j<instance->m; ++j)
    {
        sprintf(buffer, "z_%d", j);
        z[j].setName(buffer);
    }
}
...
{
    // objective function: maximize access point coverage (up to 'tau')?
    obj = IloExpr(env);
    for (int j=0; j<instance->m; ++j)
        obj += z[j];
...
bool Model::create_constraints()
{
    ...
    // 1. constraints linking z and y vars: z_j <= \sum_i {T_{ij} * y_i}  \forall j
    // NOTE: as we seek to maximize over z_j, either this constraint or the bound
    // on z_j shall be tight, i.e. z_j = \sum_i {T_{ij} * y_i} if the sum is less
    // than tau, or z_j = tau
    ...
    for (int j=0; j<instance->m; ++j)
    {
        ...
        sprintf(buffer, "linking_constr_%d", j);
...
void Model::print_status()
{
    ...
    // TODO: remove below - only printing solution and checking for errors
    if (!min_out)
    {
        cout << endl << " Solution includes " << solution.size() << " access points ";
        
        int count = 0;
        bool solution[instance->n];
        for (int j=0; j<instance->m; ++j)
        {
            // check the j-th column of T times y
            double aux = 0;
            for (int i=0; i<instance->n; ++i)
            {
                if (cplex.getValue(y[i]) > 1 - MAX_TOLERANCE)
		...
		if (j == 0)
                    solution[i] = cplex.getValue(y[i]) > (1 - MAX_TOLERANCE) ? true:false;
            }
            ...
            if (aux <= instance->tau)
            {                
                if (cplex.getValue(z[j]) < aux - MAX_TOLERANCE)
                {
                    cout << " Error! " << j << "-th contribution not consistent" << endl;
                    return;
                }
            }
            else
            {
                if (cplex.getValue(z[j]) < instance->tau - MAX_TOLERANCE)
                {
                    cout << " Error! " << j << "-th contribution not consistent" << endl;
                    return;
                }
